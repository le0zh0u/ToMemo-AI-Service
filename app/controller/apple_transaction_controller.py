    ...
    try:
        ...
        if subscription_status:
            ...
            if not rel: 
                ...
                if not key:
                    return jsonify({"successs":-1, "message":"license key create failed"}), 200    
                else :
                    return jsonify({"successs":1, "status": 1, "license_key":key}), 200
            else :
                ...
                return jsonify({"successs":1, "status": 1, "license_key":""}), 200
        ...
        print(f"出现异常 - {error}")
        return jsonify({"successs":-1, "message":"苹果单据同步失败"}), 200
